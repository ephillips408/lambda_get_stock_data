            ...
            entries.append(
                {
                    'PutRequest': {
                        'Item': {
                            # The Primary Key, example: IBM 2024-03-26
                            'pk': {'S': f'{key} {str(stock_data)}'},
                            'symbol': {'S': key},
                            'date': {'S': str(stock_data)},
                            # This is how we access the opening price
                            'open_price': {'N': data[key][str(stock_data)]['1. open']},
                            # This is how we access the closing price
                            'close_price': {'N': data[key][str(stock_data)]['4. close']},
                            # This is how we access the trade volume
                            'volume': {'N': data[key][str(stock_data)]['5. volume']}
                        }
                    }
                }
            )
...
def batch_write_stocks(db_client, table_name: str, stock_data: list):
    ...
    try:
        ...
        sublist_data = [stock_data[i: i + sublist_size]
                        for i in range(0, len(stock_data), sublist_size)]
